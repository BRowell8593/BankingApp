...
public class FullAccess extends Account implements AccountBehaviour {
    ...
    public void withdraw(Account account) {
        ...
        if (amount <= (int) balance && amount <= account.maxWithdrawal && amount >= account.overDraftLimit) {
        ...
        public void deposit(Account account){
            ...
            if (deposit <= account.maxDeposit) {
                ...
                System.out.println("You cannot deposit more than £10000");
                deposit(account);
            }
        ...
        public double checkBalance (Account account){
            System.out.println("Your current balance is £" + account.balance);
            System.out.println("This does not included your overdraft amount of £" + account.overDraftLimit );
        ...
        public void setValues (Account account){
            ...
            account.maxWithdrawal = 500;
            account.maxTransfer = 5000;
            if (!account.accType.equalsIgnoreCase("Savings")) {
                System.out.println("Would you like to add an overdraft to your account? (y/n)");
                String input = myScanner.nextLine();
                if (input.equalsIgnoreCase("Y")) {
                    if (account.accType.equalsIgnoreCase("Current")) {
                        System.out.println("A £1000 overdraft limit has been applied to your account");
                        account.overDraftLimit = 1000;
                    } else {
                        account.overDraftLimit = 2500;
                        System.out.println("A £2500 overdraft limit has been applied to your account");
                    }
                } else {
        ...
        } //setting the values for full access accounts while asking if the user would like an overdraft arranged as long as
        // the account is NOT (!) a savings account. If the user would link an overdraft and is making a current account
        // apply a £1000 overdraft limit. If a student account then apply a £2500 overdraft.
    }
